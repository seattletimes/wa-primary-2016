...
var onCounty = function(e) {
  ...
  demList.innerHTML = dem.map(r => `<li class="${r.last}"> ${r.last}: ${r.votepct.toFixed(1)}%`).join("");
  var gopList = results.querySelector(".county .republican");
  gopList.innerHTML = gop.map(r => `<li class="${r.last}"> ${r.last}: ${r.votepct.toFixed(1)}%`).join("");
};
